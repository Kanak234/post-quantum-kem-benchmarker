...
void print_header() {
  ...
  std::cout << "==============================================================="
               "==========================\n";
  std::cout << std::left << std::setw(30) << "Benchmark Operation" << std::right
            << std::setw(14) << "Iterations" << std::setw(16) << "Total (ms)"
            << std::setw(16) << "Ops / Sec" << std::setw(14) << "Avg (us)"
            << "\n";
  std::cout << "---------------------------------------------------------------"
               "--------------------------\n";
}
...
void print_row(const BenchmarkResult &res) {
  std::cout << std::left << std::setw(30) << res.operation << std::right
            << std::setw(14) << res.iterations << std::fixed
            << std::setprecision(2) << std::setw(16) << res.total_ms
...
BenchmarkResult run_benchmark(const std::string &name, size_t iterations,
  ...
  double ops_per_sec =
      (static_cast<double>(iterations) / total_ns) * 1'000'000'000.0;
...
void benchmark_kem_suite(const std::string &name, size_t iters) {
  ...
  run_benchmark(name + " Encaps", iters,
                [&]() { KemType::encaps(ct, ss, pk); });

  run_benchmark(name + " Decaps", iters,
...
int main() {
  ...
  // 1. NTT benchmarks
  {
    ...
    for (size_t i = 0; i < kem::KYBER_N; ++i)
      p.coeffs[i] = static_cast<int16_t>(i);

    run_benchmark("Poly NTT (Forward)", 20000, [&]() { kem::poly_ntt(p); });

    run_benchmark("Poly InvNTT (Inverse)", 20000,
                  [&]() { kem::poly_invntt(p); });
  }
  ...
  // 2. Symmetric Keccak / SHA3-256
  {
    ...
    run_benchmark("SHA3-256 (1 KiB)", 10000,
                  [&]() { kem::sha3_256(buf, out); });
  }
