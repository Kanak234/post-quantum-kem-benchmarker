  ...
  if (d == 4) {
    for (size_t i = 0; i < KYBER_N / 2; ++i) {
      ...
      for (size_t j = 0; j < 2; ++j) {
        ...
        if (val < 0) val = static_cast<int16_t>(val + KYBER_Q);
...
void poly_decompress(Poly& r, size_t d, std::span<const uint8_t> a) {
  if (d == 4) {
    ...
    for (size_t i = 0; i < KYBER_N / 4; ++i) {
      uint32_t t[4];
      t[0] =
          static_cast<uint32_t>(a[5 * i + 0]) | (static_cast<uint32_t>(a[5 * i + 1] & 0x03) << 8);
      ...
      t[3] = static_cast<uint32_t>(a[5 * i + 3] >> 6) | (static_cast<uint32_t>(a[5 * i + 4]) << 2);

      for (size_t j = 0; j < 4; ++j) {
        r.coeffs[4 * i + j] = static_cast<int16_t>((t[j] * KYBER_Q + 512) >> 10);
      }
    }
  } else if (d == 11) {
    for (size_t i = 0; i < KYBER_N / 8; ++i) {
      uint32_t t[8];
      t[0] =
          static_cast<uint32_t>(a[11 * i + 0]) | (static_cast<uint32_t>(a[11 * i + 1] & 0x07) << 8);
      ...
      t[7] =
          static_cast<uint32_t>(a[11 * i + 9] >> 5) | (static_cast<uint32_t>(a[11 * i + 10]) << 3);

      for (size_t j = 0; j < 8; ++j) {
        r.coeffs[8 * i + j] = static_cast<int16_t>((t[j] * KYBER_Q + 1024) >> 11);
      }
    }
  }
}
...
void poly_tobytes(std::span<uint8_t, 384> r, const Poly& a) {
  for (size_t i = 0; i < KYBER_N / 2; ++i) {
    ...
    if (static_cast<int16_t>(t0) < 0) t0 = static_cast<uint16_t>(t0 + KYBER_Q);
    if (static_cast<int16_t>(t1) < 0) t1 = static_cast<uint16_t>(t1 + KYBER_Q);
...
void poly_frombytes(Poly& r, std::span<const uint8_t, 384> a) {
  for (size_t i = 0; i < KYBER_N / 2; ++i) {
    uint16_t t0 = static_cast<uint16_t>(static_cast<uint16_t>(a[3 * i + 0]) |
                                        static_cast<uint16_t>((a[3 * i + 1] & 0x0F) << 8));
    uint16_t t1 = static_cast<uint16_t>(static_cast<uint16_t>(a[3 * i + 1] >> 4) |
...
void poly_frommsg(Poly& r, std::span<const uint8_t, 32> msg) {
  for (size_t i = 0; i < KYBER_N / 8; ++i) {
    for (size_t j = 0; j < 8; ++j) {
      int16_t mask = static_cast<int16_t>(-static_cast<int>((msg[i] >> j) & 1));
...
void poly_tomsg(std::span<uint8_t, 32> msg, const Poly& a) {
  for (size_t i = 0; i < KYBER_N / 8; ++i) {
    ...
    for (size_t j = 0; j < 8; ++j) {
      ...
      if (static_cast<int32_t>(t) < 0) t += KYBER_Q;
