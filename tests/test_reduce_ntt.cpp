...
int main() {
  std::cout << "[TEST] Running Reduction and NTT Invertibility Tests..." << std::endl;
  ...
  // Test 2: NTT and InvNTT roundtrip
  {
    ...
    std::uniform_int_distribution<int16_t> dist(-kem::KYBER_Q / 2, kem::KYBER_Q / 2);
    ...
    for (int trial = 0; trial < 100; ++trial) {
      ...
      for (size_t i = 0; i < kem::KYBER_N; ++i) {
        ...
        int16_t c1 =
            static_cast<int16_t>((orig.coeffs[i] % kem::KYBER_Q + kem::KYBER_Q) % kem::KYBER_Q);
        int16_t c2 = static_cast<int16_t>((recovered % kem::KYBER_Q + kem::KYBER_Q) % kem::KYBER_Q);
        assert(c1 == c2);
      }
    }
  ...
  // Test 3: Basemul and Convolution correctness
  {
    ...
    int16_t res = static_cast<int16_t>((c.coeffs[0] % kem::KYBER_Q + kem::KYBER_Q) % kem::KYBER_Q);
    assert(res == 35);
